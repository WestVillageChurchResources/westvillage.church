...
function hoot_attachment_id3_keys( $fields, $attachment, $context ) {
	...
	if ( 'display' === $context ) {
		...
		$fields['filesize']		 = __( 'File Size', 'dispatch' );
		$fields['mime_type']		= __( 'Mime Type', 'dispatch' );
		$fields['length_formatted'] = __( 'Run Time',  'dispatch' );
	}

	if ( hoot_attachment_is_audio( $attachment->ID ) ) {

		$fields['genre']		= __( 'Genre',	'dispatch' );
		$fields['year']		 = __( 'Year',	 'dispatch' );
		$fields['composer']	 = __( 'Composer', 'dispatch' );
		$fields['track_number'] = __( 'Track',	'dispatch' );

		if ( 'display' === $context )
			$fields['unsynchronised_lyric'] = __( 'Lyrics', 'dispatch' );
	}
