			...
			(function ($){
				$(document).ready(function() {					
					...
					<?php if(isset($_GET['libsyn_edit_post_id']) && !empty($_GET['libsyn_edit_post_id'])) { ?>
						...
						var post_redirect_url = '<?php echo admin_url('post.php').'?post='.$duplicateEditPost->post_id.'&action=edit'; ?>';
		...
		<div class="libsyn-post-form">
			<table class="form-table">
				...
				<tr valign="top" id="libsyn-post-status" style="display:none;">
					  ...
					  <td>
						<p id="libsyn-post-episode-status"><strong>Released on <?php echo date("F j, Y, g:i a", strtotime($libsyn_release_date)) ?></strong></p>
					  </td>
				</tr>
				...
				<tr valign="top">
					  ...
					  <th><label for="libsyn-post-episode"><?php _e( "Post Libsyn Episode", 'libsyn-post' ); ?></label></th>
					  <td>
		...
		if(!$isUpdatePost) {
			if($playerPlacement==='top') {
				$wpdb->update(
					$wpdb->prefix . 'posts',
					array(
						'post_content' => '[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'"]'.$download_link.wp_kses_post(self::stripShortcode('podcast', $post->post_content)),
				...
				$wpdb->update(
					$wpdb->prefix . 'posts',
					array(
						'post_content' => wp_kses_post(self::stripShortcode('podcast', $post->post_content)).'[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'"]'.$download_link,
						'post_modified' => date("Y-m-d H:i:s"),
						'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
					),
					array('ID' => $post->ID)
				...
				if (isset($matches[2]) && $matches[2] == 'podcast') { // matches (has player shortcode)
					...
					if($playerPlacement==='top') {
						$wpdb->update(
							$wpdb->prefix . 'posts',
							array(
								'post_content' => '[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'"]'.$download_link.wp_kses_post(self::stripShortcode('podcast', $new_post_content)),
								'post_modified' => date("Y-m-d H:i:s"),
								'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
							),
							array('ID' => $post->ID)
						...
						$wpdb->update(
							$wpdb->prefix . 'posts',
							array(
								'post_content' => wp_kses_post(self::stripShortcode('podcast', $new_post_content)).'[podcast src="'.$libsyn_post->url.'" height="'.$playerHeight.'" width="'.$playerWidth.'"]'.$download_link,
								'post_modified' => date("Y-m-d H:i:s"),
								'post_modified_gmt' => gmdate("Y-m-d H:i:s"),
							),
							array('ID' => $post->ID)
