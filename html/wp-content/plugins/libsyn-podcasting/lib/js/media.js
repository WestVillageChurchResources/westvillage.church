 ...
 * @since 1.0.0
 *
 * @package libsyn New Media Plugin
 * @author  Thomas Griffin
 */
jQuery(document).ready(function($){
    // Prepare the variable that holds our custom media manager.
    var libsyn_media_frame;
    
    // Bind to our click event in order to open up the new media experience.
    $(document.body).on('click.libsynOpenMediaManager', '.libsyn-open-media', function(e){
        // Prevent the default action from occuring.
        e.preventDefault();

        // If the frame already exists, re-open it.
        if ( libsyn_media_frame ) {
            libsyn_media_frame.open();
            return;
        }
         ...
         */
        libsyn_media_frame = wp.media.frames.libsyn_media_frame = wp.media({
            /**
             ...
             * the script with the object 'libsyn_nmp_media' that holds our
         ...
         */
        libsyn_media_frame.on('select', function(){
            // Grab our attachment selection and construct a JSON representation of the model.
            var media_attachment = libsyn_media_frame.state().get('selection').first().toJSON();

            // Send the attachment URL to our custom input field via jQuery.
            $('#libsyn-new-media-image').val(media_attachment.url);
        });
