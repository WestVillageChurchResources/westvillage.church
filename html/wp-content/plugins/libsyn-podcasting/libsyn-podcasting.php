...
Version: 0.9.6.1
Author: Libsyn
Author URI: http://www.libsyn.com
License: GPLv3
*/
...
if ( ! function_exists( 'libsyn_unqprfx_embed_shortcode' ) ) :
	...
	include_once( ABSPATH . 'wp-admin/includes/plugin.php' ); //include plugin.php to run is_plugin_active() check
	//wp helper functions: is_plugin_active(), is_multisite(), is_plugin_active_for_network(), $plugin_list = get_option('active_plugins');
	...
	if( is_plugin_active(LIBSYN_DIR.'/'.LIBSYN_DIR.'.php') ) {
		...
		add_action( 'template_redirect', 'Libsyn\\Utilities::checkAjax' );
		
		//oauth settings save
		add_filter('query_vars', 'Libsyn\\Utilities::plugin_add_trigger_libsyn_oauth_settings');
		add_action( 'template_redirect', 'Libsyn\\Utilities::saveOauthSettings' );
		
		//clear settings
		add_filter('query_vars', 'Libsyn\\Utilities::plugin_add_trigger_libsyn_update_oauth_settings');
		add_action( 'template_redirect', 'Libsyn\\Utilities::updateOauthSettings' );
